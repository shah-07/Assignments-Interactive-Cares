<nav class="border-b border-emerald-300 border-opacity-25 bg-emerald-600"
            ...
            <div class="flex items-center px-2 lg:px-0">
                <div class="hidden sm:block">
                    <div class="flex space-x-4">
                        ...
                        <a href="./dashboard"
                            class="<?= urlIs('/dashboard') ? 'bg-emerald-700' : 'hover:bg-emerald-500 hover:bg-opacity-75'; ?> text-white rounded-md py-2 px-3 text-sm font-medium"
                            aria-current="page">Dashboard</a>
                        <a href="./deposit"
                            class="<?= urlIs('/deposit') ? 'bg-emerald-700' : 'hover:bg-emerald-500 hover:bg-opacity-75'; ?> text-white rounded-md py-2 px-3 text-sm font-medium">Deposit</a>
                        <a href="./withdraw"
                            class="<?= urlIs('/withdraw') ? 'bg-emerald-700' : 'hover:bg-emerald-500 hover:bg-opacity-75'; ?> text-white rounded-md py-2 px-3 text-sm font-medium">Withdraw</a>
                        <a href="./transfer"
                            class="<?= urlIs('/transfer') ? 'bg-emerald-700' : 'hover:bg-emerald-500 hover:bg-opacity-75'; ?> text-white rounded-md py-2 px-3 text-sm font-medium">Transfer</a>
                    </div>
                </div>
            </div>
                    ...
                    <div>
                        ...
                        alt="Ahmed Shamim Hasan Shaon" /> -->
                            <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100">
                                <span class="font-medium leading-none text-emerald-700"><?php
                                $name = isset($_COOKIE['username']) ? $_COOKIE['username'] : 'Guest User';
                                $firstLetters = '';
                                foreach (explode(' ', $name) as $word) {
                                    $firstLetters .= strtoupper(substr($word, 0, 1)); // Append the first letter of each word
                                }
                                echo $firstLetters;
                                ?></span>
                            </span>
    ...
    <div x-show="mobileMenuOpen" class="sm:hidden" id="mobile-menu">
        <div class="space-y-1 pt-2 pb-3">
            <a href="./dashboard" class="bg-emerald-700 text-white block rounded-md py-2 px-3 text-base font-medium"
                aria-current="page">Dashboard</a>
            ...
            <div class="flex items-center px-5">
                ...
                <div class="ml-3">
                    <div class="text-base font-medium text-white">
                        <?php echo isset($_COOKIE['username']) ? htmlspecialchars($_COOKIE['username']) : 'Guest'; ?>
                    </div>
                    <div class="text-sm font-medium text-emerald-300">
                        <?php echo isset($_COOKIE['email']) ? htmlspecialchars($_COOKIE['email']) : 'No email provided'; ?>
                    </div>
                </div>
                <button type="button"
